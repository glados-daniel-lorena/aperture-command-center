...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    sql += ' ORDER BY n.created_at ASC LIMIT ?';
    params.push(limit);

    const undeliveredNotifications = (await query(sql, params)).rows as (Notification & { session_key?: string })[];
      ...
      try {
        ...
        if (!dry_run) {
          ...
          try {
            ...
            await query('UPDATE notifications SET delivered_at = ? WHERE id = ? AND workspace_id = ?', [now, notification.id, workspaceId]);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let baseSql = 'SELECT COUNT(*) as count FROM notifications WHERE workspace_id = ?';
    let params: any[] = [workspaceId];

    if (agent) {
      baseSql += ' AND recipient = ?';
      params.push(agent);
    }

    const totalNotifications = (await query(baseSql, params)).rows[0] as { count: number };
    const undeliveredCount = (await query(baseSql + ' AND delivered_at IS NULL', params)).rows[0] as { count: number };
    const deliveredCount = (await query(baseSql + ' AND delivered_at IS NOT NULL', params)).rows[0] as { count: number };
    ...
    const recentDeliveries = (await query(`
      ...
      LIMIT 10
    `, agent ? [workspaceId, agent] : [workspaceId])).rows;

    // Get agents with pending notifications
    const agentsPending = (await query(`
      SELECT
        n.recipient,
        a.session_key,
