...
async function ensureUniqueUsername(base: string): Promise<string> {
  let candidate = base
  let i = 0
  while ((await query('SELECT 1 FROM users WHERE username = ?', [candidate])).rows[0]) {
    i += 1
...
export async function GET(request: NextRequest) {
  ...
  const { rows } = status === 'all'
    ? await query(
        `SELECT * FROM access_requests ORDER BY CASE WHEN status = 'pending' THEN 0 ELSE 1 END, last_attempt_at DESC, id DESC`
      )
    : await query(
        'SELECT * FROM access_requests WHERE status = ? ORDER BY last_attempt_at DESC, id DESC',
        [status]
      )
...
export async function POST(request: NextRequest) {
  ...
  const reqRow = (await query('SELECT * FROM access_requests WHERE id = ?', [requestId])).rows[0] as any
  ...
  if (action === 'reject') {
    await query(
      `UPDATE access_requests SET status = 'rejected', reviewed_by = ?, reviewed_at = ?, review_note = ? WHERE id = ?`,
      [admin.username, now, note, requestId]
    )
  ...
  if (existing) {
    await query(
      `UPDATE users SET provider = 'google', provider_user_id = ?, email = ?, avatar_url = COALESCE(?, avatar_url), is_approved = 1, role = ?, approved_by = ?, approved_at = ?, updated_at = ? WHERE id = ?`,
      [providerUserId, email, avatarUrl, role, admin.username, now, now, existing.id]
    )
  ...
  await query(
    `UPDATE access_requests SET status = 'approved', reviewed_by = ?, reviewed_at = ?, review_note = ?, approved_user_id = ? WHERE id = ?`,
    [admin.username, now, note, userId, requestId]
  )
