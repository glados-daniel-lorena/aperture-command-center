...
async function buildIdentity(agent: any, workspaceId: number) {
  ...
  `, [agent.name, workspaceId])).rows[0] as any;

  const commentCountRow = (await query(
...
async function buildAuditTrail(agentName: string, workspaceId: number, since: number) {
  const activities = (await query(`
    ...
    LIMIT 200
  `, [agentName, workspaceId, since])).rows as any[];
  ...
  try {
    auditEntries = (await query(`
      ...
      ORDER BY created_at DESC
      LIMIT 100
    `, [agentName, `%${agentName}%`, since])).rows as any[];
  } catch {
...
async function buildMutations(agentName: string, workspaceId: number, since: number) {
  const taskMutations = (await query(`
    SELECT id, type, entity_type, entity_id, description, data, created_at
    FROM activities
    WHERE actor = ? AND workspace_id = ? AND created_at >= ?
  ...
  const comments = (await query(`
    ...
    LIMIT 50
  `, [workspaceId, agentName, workspaceId, since])).rows as any[];
  ...
  const statusChanges = (await query(`
    ...
    ORDER BY created_at DESC
    LIMIT 50
  `, [workspaceId, since, agentName, `%${agentName}%`])).rows as any[];
...
async function buildCostAttribution(agentName: string, workspaceId: number, since: number) {
  try {
    ...
    const byModelAlt = (await query(`
      ...
      GROUP BY model
      ORDER BY (input_tokens + output_tokens) DESC
    `, [`${agentName}:%`, agentName, workspaceId, since])).rows as Array<{
      model: string; request_count: number; input_tokens: number; output_tokens: number
    }>;
    ...
    `, [agentName, `${agentName}:%`, workspaceId, since])).rows as any[];
