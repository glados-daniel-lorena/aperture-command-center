...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const templates = (await query('SELECT id, name FROM workflow_templates', [])).rows as Array<{ id: number; name: string }>
    ...
    `, [workspaceId])).rows as Array<{ pipeline_id: number; total: number; completed: number; failed: number; running: number }>
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existing = (await query(
      `SELECT id FROM workflow_templates WHERE id IN (${placeholders})`,
      templateIds
    ...
    const insertResult = await query(`
      ...
      VALUES (?, ?, ?, ?, ?)
      RETURNING id
    `, [name, description || null, JSON.stringify(cleanSteps), auth.user?.username || 'system', workspaceId])
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await query(`UPDATE workflow_pipelines SET ${fields.join(', ')} WHERE id = ? AND workspace_id = ?`, params)
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    await query('DELETE FROM workflow_pipelines WHERE id = ? AND workspace_id = ?', [parseInt(id), workspaceId])
