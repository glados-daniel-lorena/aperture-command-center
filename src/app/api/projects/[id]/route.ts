...
export async function PATCH(
  ...
  try {
    ...
    const current = (await query(`SELECT * FROM projects WHERE id = ? AND workspace_id = ?`, [projectId, workspaceId])).rows[0] as any
    ...
    if (typeof body?.ticket_prefix === 'string' || typeof body?.ticketPrefix === 'string') {
      ...
      `, [workspaceId, prefix, projectId])).rows[0]
...
export async function DELETE(
  ...
  try {
    ...
    if (mode !== 'delete') {
      await query(
        `UPDATE projects SET status = 'archived', updated_at = EXTRACT(EPOCH FROM NOW())::INTEGER WHERE id = ? AND workspace_id = ?`,
    ...
    await withTransaction(async (txQuery) => {
      ...
      await txQuery(`DELETE FROM projects WHERE id = ? AND workspace_id = ?`, [projectId, workspaceId])
    })
