...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const exists = (await query(`
      ...
      LIMIT 1
    `, [workspaceId, slug, ticketPrefix])).rows[0] as { id: number } | undefined
    if (exists) {
    ...
    const insertResult = await query(`
      ...
      VALUES (?, ?, ?, ?, ?, 'active', EXTRACT(EPOCH FROM NOW())::INTEGER, EXTRACT(EPOCH FROM NOW())::INTEGER)
      RETURNING id
    `, [workspaceId, name, slug, description || null, ticketPrefix])
