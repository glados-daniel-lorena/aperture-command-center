...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tenant = (await query('SELECT * FROM tenants WHERE id = ?', [tenantId])).rows[0]
    if (!tenant) {
    ...
    const insertResult = await query(`
      ...
      VALUES (?, ?, 'queued', ?, ?, ?, ?, EXTRACT(EPOCH FROM NOW())::INTEGER)
      RETURNING id
    `, [
      tenantId,
      jobType,
    ...
    const job = (await query('SELECT * FROM provision_jobs WHERE id = ?', [id])).rows[0]
